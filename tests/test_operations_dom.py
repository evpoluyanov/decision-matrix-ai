...
def test_product_analytics_runs_only_when_server_renders_an_allowed_counter():
    node = shutil.which("node")
    if not node:
        pytest.skip("Node.js is required for JS contract tests")
    template = Environment(loader=FileSystemLoader("app/templates")).get_template(
        "_product_analytics.html"
    )
    assert template.render(product_analytics_counter_id=None).strip() == ""
    html = template.render(product_analytics_counter_id=12345)
    script = re.search(r"<script>(.*?)</script>", html, re.S).group(1)
    check = """
const assert=require('node:assert/strict'),vm=require('node:vm');
const script=SCRIPT;
function run(loaded=false){
 const calls=[],scripts=[];const c={document:{createElement:()=>({}),head:{appendChild:e=>scripts.push(e)}}};
...
const fresh=run();assert.equal(fresh.scripts.length,1);
...
assert.equal(fresh.c.ym.a.find(x=>x[1]==='reachGoal')[2],'feedback_submitted');
const existing=run(true);assert.equal(existing.scripts.length,0);assert.equal(existing.calls.length,1);
